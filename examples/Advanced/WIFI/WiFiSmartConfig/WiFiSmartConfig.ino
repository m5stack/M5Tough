...
* date：2021/7/30
*******************************************************************************
 *  Fill in WIFI configuration information through mobile APP to connect M5Tough to relevant WIFI
 *  通过手机APP填写WIFI配置信息使 M5Tough连接至相关WIFI
...
void setup() {
  ...
  //Wait for the M5Tough to receive network information from the phone
  //等待M5Tough接收到来自手机的配网信息
  ...
  while (WiFi.status() != WL_CONNECTED) { //M5Tough will connect automatically upon receipt of the configuration information, and return true if the connection is successful.  收到配网信息后M5Tough将自动连接，若连接成功将返回true
    delay(500);
    M5.Lcd.print(".");
  }
  M5.Lcd.print("\nWiFi Connect To: ");
