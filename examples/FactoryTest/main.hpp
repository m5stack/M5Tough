...
void setSpeaker(int sampleRate = 16000) {
  ...
  res = i2s_set_clk(Speak_I2S_NUMBER, (uint32_t)sampleRate,
...
static void IRAM_ATTR soundTask(void* sound_param) {
  ...
  for (;;) {
    ...
    for (int i = 0; i < 2; ++i) {
      i2s_write(Speak_I2S_NUMBER, data, I2S_DATA_LEN * 2, &bytes_written,
                portMAX_DELAY);
    }
...
bool contain(int x, int y, int w, int h) {
  return x <= tp->x && tp->x < (x + w) && y <= tp->y && tp->y < (y + h);
}
